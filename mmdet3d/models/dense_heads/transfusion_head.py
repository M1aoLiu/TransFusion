...
class TransFusionHead(nn.Module):
            ...
            self.decoder.append(
                TransformerDecoderLayer(  # decoder解码层
                    ...
                    self_posembed=PositionEmbeddingLearned(
                        2, hidden_channel
                    ),  # self-attention的位置编码层，输入维度2(此时都在BEV下，只有x,y维度)，输出维度128
                    cross_posembed=PositionEmbeddingLearned(
                        2, hidden_channel
                    ),  # cross-attention的位置编码层，输入维度2，输出维度128
                )
            )
    ...
    def create_2D_grid(
        self, x_size, y_size
    ):  # 2.1.1 将pts的图像大小划分为若干BEV网格并返回坐标
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        batch_size = inputs.shape[
            0
        ]  # pts:-B, 512, H(180), W(180)] imgs: [B, 256, H(112), W(200)] imgs和pts的H，W大小不同
        ...
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(
            lidar_feat.device
        )  # 得到pts图像在BEV视角下的BEV网格坐标(B, H*W, 2(x,y))

        if self.fuse_img:
            img_feat = self.shared_conv_img(
                img_inputs
            )  # [BS * n_views, C(128), H, W] 此时imgs和pts的C相同
            ...
            )  # 进行一次全连接层提取特征 [BS, C, H, n_views*W]

            # positional encoding for image guided query initialization
            if (
                self.img_feat_collapsed_pos is None
            ):  # 得到img图像在BEV视角下的BEV网格坐标(B, W*views, 2(x,y))
                img_feat_collapsed_pos = self.img_feat_collapsed_pos = (
                    self.create_2D_grid(1, img_feat_collapsed.shape[-1]).to(
                        img_feat.device
                    )
                )
            else:
        ...
        if self.initialize_by_heatmap:
            ...
            if self.fuse_img:
                ...
                heatmap = ( # 热力图相加/2求平均
                    dense_heatmap.detach().sigmoid()
                    + dense_heatmap_img.detach().sigmoid()
                ) / 2
            else:
        ...
        if self.fuse_img:
            ...
            query_pos_3d = (
                torch.cat([query_pos_realmetric, res_layer["height"]], dim=1)
                .detach()
                .clone()
            )
            if "vel" in res_layer:
            ...
            on_the_image_mask = (
                torch.ones([batch_size, self.num_proposals]).to(query_pos_3d.device)
                * -1
            )
            ...
            for sample_idx in range(batch_size if self.fuse_img else 0):
                ...
                for view_idx in range(self.num_views):
                    ...
                    h, w = img_pad_shape
                    on_the_image = (
                        (coor_x > 0) * (coor_x < w) * (coor_y > 0) * (coor_y < h)
                    )
                    ...
                    distance = (centers[:, None, :] - (img_feat_pos - 0.5)).norm(
                        dim=-1
                    ) ** 2
    ...
    def forward(self, feats, img_feats, img_metas):
        ...
        if img_feats is None:
            img_feats = [None]
        res = multi_apply(
            self.forward_single, feats, img_feats, [img_metas]
        )  # 2.1 TransFusionHead得到检测结果
    ...
    def get_bboxes(
        self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False
    ):
